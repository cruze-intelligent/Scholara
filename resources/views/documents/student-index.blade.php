<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Medical Documents') }} — {{ $student->first_name }} {{ $student->last_name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (auth()->user()->hasAnyRole(['nurse', 'admin']))
                <x-card>
                    <form method="POST" action="{{ route('students.documents.store', $student) }}" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <x-form.input name="title" label="Title" placeholder="e.g. Asthma inhaler dosage sheet" />
                        <div>
            ...
            <x-card>
                ...
                @empty
                    <x-empty-state message="No medical documents attached yet." />
                @endforelse
            </x-card>
        </div>
    </div>
